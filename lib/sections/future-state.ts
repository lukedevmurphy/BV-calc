...
export function futureStateSection(ctx: ProposalContext): SectionOutput {
  ...
  return {
    ...
    kind: "future_state",
    title: "Future State",
    subtitle: "Not fewer people — the same experts spending more hours on higher-judgment work",
    table: {
    ...
    speakerNotes:
      ...
      `Cycle times drop from days to hours — the constraint becomes decision quality, not production capacity. Output becomes ` +
      `consistent: the same standards, sources, and disclosures applied every time. Capacity scales with usage, not headcount — ` +
      `consumption pricing means leverage, not licenses. Walk one workflow end-to-end in the to-be pattern. The ` +
      `automation/augmentation/agency framing sets expectations honestly: automation rows ship fastest, agency rows need ` +
      `guardrails and earn trust over the roadmap phases.`,
